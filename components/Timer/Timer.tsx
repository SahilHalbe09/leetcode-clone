...
const Timer: React.FC<TimerProps> = () => {
	...
	return (
		<>
			{showTimer ? (
				<div className="flex items-center space-x-2 bg-dark-fill-2 p-1.5 cursor-pointer rounded hover:bg-dark-fill-2">
					<div>{formatTime(time)}</div>
					<FiRefreshCw
						className="hover:text-white hover:rotate-180 transition-all ease-in-out duration-300"
						onClick={() => {
							setShowTimer(false);
							setTime(0);
						}}
				...
				<div className="flex items-center p-1 h-8 hover:bg-dark-fill-3 rounded cursor-pointer" onClick={() => setShowTimer(true)}>
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="1em" height="1em" fill="currentColor" className="h-6 w-6">
						<path
							fillRule="evenodd"
