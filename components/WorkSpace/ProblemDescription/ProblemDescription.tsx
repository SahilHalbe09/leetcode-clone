	...
	const handleLike = async () => {
		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (userDoc.exists() && problemDoc.exists()) {
				// If already liked, and then clicked like btn:
				if (liked) {
					transaction.update(userRef, {
						likedProblems: userDoc.data().likedProblems.filter((id: string) => id !== problemDoc.id),
					});
					...
					// If already disliked, and then clicked like btn:
				} else if (disliked) {
					transaction.update(userRef, {
					...
					// If first time updating the likes:
				} else {
					transaction.update(userRef, {
						likedProblems: [...userDoc.data().likedProblems, problem.id],
					});
	...
	const handleDislike = async () => {
		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (userDoc.exists() && problemDoc.exists()) {
				...
				if (disliked) {
					transaction.update(userRef, {
						dislikedProblems: userDoc.data().dislikedProblems.filter((id: string) => id !== problemDoc.id),
					});
						...
						{!loading && currentProblem && (
							<div className="flex items-center mt-3">
								...
								<div
									...
									onClick={handleLike}
								>
									{liked ? <AiFillLike className="text-dark-green-s" /> : <AiFillLike />}

									{/* {liked && !updatingLikes && <AiFillLike className="text-dark-green-s" />}
								...
								>
									{disliked ? <AiFillDislike className="text-dark-pink" /> : <AiFillDislike />}

									{/* {disliked && !updatingDislikes && <AiFillDislike className="text-dark-pink" />}
									{!disliked && !updatingDislikes && <AiFillDislike />}
									{updatingDislikes && <AiOutlineLoading3Quarters className="animate-spin" />} */}
								...
								<div className="cursor-pointer hover:bg-dark-fill-3  rounded p-[3px]  ml-4 text-xl transition-colors duration-200 text-green-s text-dark-gray-6" onClick={handleStar}>
									{starred ? <TiStarFullOutline className="text-dark-yellow" /> : <TiStarOutline />}
								</div>
							</div>
						)}
						...
						<div className="mt-4">
							{problem.examples.map((example, index) => (
								<div key={example.id}>
									...
									{example.img && <Image src={example.img} width={500} height={100} alt="" className="mt-3" />}
...
function useGetUserDataProblem(problemId: string) {
